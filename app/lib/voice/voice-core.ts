...
/**
 * Identify the patient for this call by phone number alone — no CallLog
 * round-trip. The agent carries the phone number forward in conversation
 * memory and passes it as `caller_phone` on every write tool call.
 */
...
const NOT_IDENTIFIED = {
  message: 'I need your mobile number to find your record. Could you tell me the 10-digit number you registered with, or register with a new one?',
};
...
export async function registerPatient(ctx: VoiceCtx, args: { fullName?: string; email?: string; phone?: string }) {
  ...
  // Silent dedupe: a caller who already has a record just gets it reused —
  // never surfaced to them as an "are you new or existing?" question.
  const existing = await db.oPD_REG.findFirst({ where: { phone: { contains: phone } }, select: { patient_id: true, full_name: true, phone: true, email: true } });
  if (existing) {
    return {
      patientId: existing.patient_id,
      message: `I have ${existing.full_name}, mobile ${existing.phone}${existing.email ? `, email ${existing.email}` : ''} on file. Which doctor and time would you like to book?`,
    };
  }
 ...
 * CRMLead for staff follow-up — visible on the existing /crm/leads page.
 */
export async function requestCallback(ctx: VoiceCtx, args: { reason?: string; department?: string; name?: string; phone?: string }) {
  const db = getTenantPrisma(ctx.organizationId);

  const phone = normalizePhone(args.phone ?? ctx.callerPhone);
  const name = args.name?.trim() || 'Phone caller';

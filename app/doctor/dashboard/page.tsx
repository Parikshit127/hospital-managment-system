...
import {
    ...
    History, User, Clipboard, Printer, RefreshCw, HeartPulse, LogOut, Trash2,
...
export default function DoctorDashboard() {
    ...
    const handleSaveNotes = () => withSubmission(async () => {
        ...
        if (activePatient.status === 'Admitted') {
            ...
            await saveClinicalNotes({ patient_id: activePatient.patient_id, appointment_id: activePatient.appointment_id, diagnosis, notes, doctor: 'Dr. Sarah' });
    ...
    const handleOrderLab = () => withSubmission(async () => {
        ...
        await orderLabTest({ patient_id: activePatient.patient_id, test_type: selectedTest, doctor_id: 'doc1' });
    ...
    const handleWalkinSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.success) { alert(`Walk-in Registered! Patient ID: ${res.patient_id}`); setShowWalkinModal(false); const q = await getPatientQueue(); if (q.success) setQueue(q.data as any); }
    ...
    // ─── INPUT STYLES ───
    const inputCls = "w-full p-3.5 bg-white/5 border border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500/30 outline-none font-medium text-white placeholder:text-white/15";
    const labelCls = "text-[10px] font-black text-white/30 uppercase tracking-[0.15em] ml-1 block mb-1.5";
        ...
        <div className="flex h-screen bg-[#0B0F1A] font-sans text-white overflow-hidden relative">
            ...
            {showDischargeModal && dischargePdfUrl && (
                <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-gradient-to-br from-[#131A2E] to-[#0F1425] w-full max-w-4xl h-[85vh] rounded-2xl shadow-2xl border border-white/10 overflow-hidden flex flex-col">
                        <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center shrink-0">
                            ...
                            <button onClick={() => setShowDischargeModal(false)} className="text-white/30 hover:text-white/60 bg-white/5 p-2 rounded-full hover:bg-white/10"><X className="h-5 w-5" /></button>
                        </div>
                        <div className="flex-1 bg-[#0B0F1A] p-0 relative"><iframe src={dischargePdfUrl} className="w-full h-full" title="Discharge Summary PDF"></iframe></div>
                        <div className="p-4 border-t border-white/5 flex justify-end gap-3 shrink-0">
                            <button onClick={() => setShowDischargeModal(false)} className="px-6 py-2.5 font-bold text-white/30 hover:text-white/60 rounded-xl">Close</button>
            ...
            {showWalkinModal && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-gradient-to-br from-[#131A2E] to-[#0F1425] rounded-2xl shadow-2xl w-full max-w-lg border border-white/10 overflow-hidden">
                        <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center">
                            ...
                            <button onClick={() => setShowWalkinModal(false)} className="text-white/30 hover:text-white/60"><X className="h-5 w-5" /></button>
                        </div>
                        <form onSubmit={handleWalkinSubmit} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <select className={inputCls} value={walkinFormData.gender} onChange={e => setWalkinFormData({ ...walkinFormData, gender: e.target.value })}><option className="bg-[#131A2E]">Male</option><option className="bg-[#131A2E]">Female</option><option className="bg-[#131A2E]">Other</option></select>
                                <select className={inputCls} value={walkinFormData.department} onChange={e => setWalkinFormData({ ...walkinFormData, department: e.target.value })}><option className="bg-[#131A2E]">General</option><option className="bg-[#131A2E]">Cardiology</option><option className="bg-[#131A2E]">Orthopedics</option></select>
                            </div>
            ...
            {showAdmitModal && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-gradient-to-br from-[#131A2E] to-[#0F1425] rounded-2xl shadow-2xl w-full max-w-md border border-white/10 overflow-hidden">
                        <div className="px-6 py-4 border-b border-white/5 flex justify-between items-center">
                            <h3 className="font-black text-lg flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-rose-400" /> Admit Patient</h3>
                            <button onClick={() => setShowAdmitModal(false)} className="text-white/30 hover:text-white/60"><X className="h-5 w-5" /></button>
                        </div>
                        <div className="p-6 space-y-4">
                            <p className="text-sm text-white/50">Enter provisional diagnosis to admit <strong className="text-white/80">{activePatient?.full_name}</strong>.</p>
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden print-area">
                        <div className="p-8 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                            ...
                            <div className="text-right"><h3 className="font-bold text-lg text-slate-900">Avani Hospital</h3><p className="text-xs text-slate-500">Dr. Sarah • {new Date().toLocaleDateString()}</p></div>
                        </div>
            ...
            {/* ── LEFT SIDEBAR — QUEUE ── */}
            <aside className="w-80 flex flex-col border-r border-white/5 bg-[#0F1425]/90 backdrop-blur-xl">
                <div className="p-5 border-b border-white/5">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-black text-white/70 flex items-center gap-2 text-sm"><Users className="h-4 w-4 text-teal-400" /> Patient Queue</h3>
                    ...
                    <div className="relative group">
                        ...
                        <input type="text" placeholder="Search patient..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-teal-500/20 outline-none transition-all placeholder:text-white/20 font-medium text-white" />
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                    {loading ? <div className="text-center p-8 text-white/30 animate-pulse font-bold">Loading queue...</div> : (
                        filteredQueue.length === 0 ? (
                            <div className="text-center p-12 text-white/25 text-sm flex flex-col items-center gap-2"><Users className="h-8 w-8 text-white/10" />No patients found</div>
                        ) : filteredQueue.map((p) => (
                            <div key={p.patient_id} onClick={isSubmitting ? undefined : () => setActivePatient(p)}
                                className={`p-4 rounded-xl cursor-pointer transition-all border group ${activePatient?.patient_id === p.patient_id ? 'bg-teal-500/10 border-teal-500/30 ring-1 ring-teal-500/30' : 'bg-white/[0.02] hover:bg-white/[0.04] border-white/5 hover:border-teal-500/20'} ${isSubmitting ? 'opacity-50 pointer-events-none' : ''}`}>
                                <div className="flex justify-between items-start mb-1">
                                    ...
                                    <span className="text-[10px] text-white/20 font-mono">#{p.digital_id ? p.digital_id : p.patient_id.slice(0, 4)}</span>
                                </div>
                                <h4 className="font-bold text-sm truncate text-white/80 group-hover:text-teal-400 transition-colors">{p.full_name}</h4>
                                <div className="flex gap-2 mt-1 items-center">
                                    <span className="text-[10px] text-white/30">{p.age ? `${p.age}y` : ''}{p.gender ? ` / ${p.gender}` : ''}</span>
                                    <span className="text-[10px] text-white/15">•</span>
                                    <span className="text-[10px] text-white/30">{p.department || 'General'}</span>
                                </div>
                                {p.reason_for_visit && (
                ...
                <div className="p-4 border-t border-white/5">
                    <button onClick={() => setShowWalkinModal(true)} disabled={isSubmitting} className="w-full py-3 border-2 border-dashed border-white/10 rounded-xl text-white/30 font-bold hover:bg-white/5 hover:text-teal-400 hover:border-teal-500/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            <main className="flex-1 flex flex-col min-w-0 relative">
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                    <div className="absolute top-1/4 -right-32 w-96 h-96 bg-teal-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDuration: '5s' }} />
                    <div className="absolute bottom-1/4 -left-32 w-96 h-96 bg-violet-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDuration: '7s' }} />
                </div>

                <header className="bg-[#0F1425]/90 backdrop-blur-xl border-b border-white/5 h-16 px-6 flex items-center justify-between shrink-0 z-10 relative">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="absolute inset-0 bg-gradient-to-br from-teal-400 to-emerald-600 rounded-xl blur-md opacity-50" />
                            <div className="relative bg-gradient-to-br from-teal-400 to-emerald-600 p-2 rounded-xl shadow-lg shadow-teal-500/20"><Activity className="h-5 w-5 text-white" /></div>
                        </div>
                        <div>
                            <h1 className="font-black text-lg tracking-tight bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent">Doctor&apos;s Console</h1>
                            <p className="text-[10px] font-bold text-teal-400 uppercase tracking-[0.2em]">Clinical Workspace</p>
                        </div>
                    </div>
                    <Link href="/login" className="flex items-center gap-2 text-xs font-bold text-rose-400 hover:text-rose-300 px-3 py-2 rounded-lg hover:bg-rose-500/10 transition-all"><LogOut className="h-3.5 w-3.5" /> Logout</Link>
                </header>

                {activePatient ? (
                    <div className="flex-1 overflow-y-auto p-6 z-10 relative">
                        {/* Patient Header Card */}
                        <div className="bg-gradient-to-br from-[#131A2E] to-[#0F1425] border border-white/5 rounded-2xl p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:border-teal-500/20 transition-all">
                            <div className="flex items-center gap-5">
                                <div className="h-14 w-14 bg-gradient-to-br from-violet-500/20 to-indigo-500/20 rounded-2xl border border-white/10 flex items-center justify-center"><User className="h-7 w-7 text-violet-400" /></div>
                                <div>
                                    <div className="flex items-center gap-3">
                                        <h1 className="text-2xl font-black text-white tracking-tight">{activePatient.full_name}</h1>
                                    ...
                                    <div className="flex gap-3 mt-2 text-xs text-white/40 font-medium flex-wrap">
                                        {activePatient.age && <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-lg border border-white/5"><User className="h-3 w-3" /> {activePatient.age}y{activePatient.gender ? ` / ${activePatient.gender}` : ''}</span>}
                                        <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-lg border border-white/5"><Clock className="h-3 w-3" /> {new Date(activePatient.created_at).toLocaleTimeString()}</span>
                                        <span className="flex items-center gap-1.5 bg-white/5 px-2 py-0.5 rounded-lg border border-white/5"><Stethoscope className="h-3 w-3" /> {activePatient.department}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex gap-3 items-center">
                                <select value={activePatient.status || 'Pending'} onChange={(e) => handleStatusUpdate(e.target.value)} disabled={isSubmitting} className="bg-white/5 border border-white/10 text-white text-sm rounded-xl focus:ring-teal-500 p-2.5 font-bold outline-none appearance-none">
                                    {['Scheduled', 'Checked In', 'In Progress', 'Completed', 'Cancelled', 'Admitted'].map(s => <option key={s} value={s} className="bg-[#131A2E]">{s}</option>)}
                                </select>
                        ...
                        {/* Tabs */}
                        <div className="bg-gradient-to-br from-[#131A2E] to-[#0F1425] border border-white/5 rounded-2xl min-h-[500px] flex flex-col overflow-hidden">
                            <div className="flex border-b border-white/5 px-2 pt-2 overflow-x-auto">
                                {(['triage', 'notes', 'history', 'lab', 'pharmacy'] as const).map((tab) => (
                                    <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-4 text-sm font-bold border-b-2 flex items-center gap-2 transition-all outline-none whitespace-nowrap ${activeTab === tab ? 'border-teal-400 text-teal-400 bg-teal-500/5 rounded-t-lg' : 'border-transparent text-white/30 hover:text-white/60 rounded-t-lg'}`}>
                            ...
                            <div className="p-8 flex-1">
                                ...
                                {activeTab === 'triage' && (
                                    <div className="max-w-4xl space-y-6">
                                        {loadingTriage ? (
                                            <div className="text-center py-16 text-white/30 font-bold flex flex-col items-center gap-3"><Loader2 className="h-8 w-8 animate-spin text-teal-400" />Loading AI Assessment...</div>
                                        ) : !triageData ? (
                                            <div className="bg-white/5 border border-dashed border-white/10 rounded-2xl p-12 text-center">
                                                ...
                                                <h3 className="font-black text-white/50 text-lg mb-1">No AI Assessment Found</h3>
                                                <p className="text-white/25 text-sm font-medium">This patient was registered without AI triage. Proceed to Clinical Notes.</p>
                                            </div>
                                        ) : (
                                            <>
                                                {/* Triage Level Banner */}
                                                <div className={`rounded-2xl p-5 border flex items-center justify-between ${triageData.triageLevel === 'Emergency' ? 'bg-red-500/5 border-red-500/20' : triageData.triageLevel === 'Urgent' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-emerald-500/5 border-emerald-500/20'}`}>
                                                    ...
                                                    <div className="text-right">
                                                        <span className="text-[10px] font-black text-white/30 uppercase tracking-wider block">Department</span>
                                                        <span className="text-sm font-black text-white/80">{triageData.recommendedDepartment}</span>
                                                        <span className="text-[10px] text-white/20 block mt-0.5">{triageData.triageDate ? new Date(triageData.triageDate).toLocaleString() : ''}</span>
                                                    </div>
                                                </div>
                                                ...
                                                {triageData.vitals && (
                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                                        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-center hover:border-teal-500/20 transition-all">
                                                            <HeartPulse className="h-5 w-5 text-rose-400 mx-auto mb-2" />
                                                            <span className="text-[10px] font-black text-white/30 uppercase block">Blood Pressure</span>
                                                            <span className="text-lg font-black text-white/80">{triageData.vitals.bloodPressure || 'N/A'}</span>
                                                        </div>
                                                        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-center hover:border-teal-500/20 transition-all">
                                                            <Heart className="h-5 w-5 text-pink-400 mx-auto mb-2" />
                                                            <span className="text-[10px] font-black text-white/30 uppercase block">Heart Rate</span>
                                                            <span className="text-lg font-black text-white/80">{triageData.vitals.heartRate ? `${triageData.vitals.heartRate} BPM` : 'N/A'}</span>
                                                        </div>
                                                        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-center hover:border-teal-500/20 transition-all">
                                                            <Thermometer className="h-5 w-5 text-orange-400 mx-auto mb-2" />
                                                            <span className="text-[10px] font-black text-white/30 uppercase block">Temperature</span>
                                                            <span className="text-lg font-black text-white/80">{triageData.vitals.temperature ? `${triageData.vitals.temperature}°C` : 'N/A'}</span>
                                                        </div>
                                                        <div className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-center hover:border-teal-500/20 transition-all">
                                                            <Wind className="h-5 w-5 text-cyan-400 mx-auto mb-2" />
                                                            <span className="text-[10px] font-black text-white/30 uppercase block">SpO2</span>
                                                            <span className="text-lg font-black text-white/80">{triageData.vitals.oxygenSat ? `${triageData.vitals.oxygenSat}%` : 'N/A'}</span>
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                                    ...
                                                    <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
                                                        <h4 className="font-black text-white/60 text-xs uppercase tracking-wider mb-3 flex items-center gap-2"><Activity className="h-4 w-4 text-teal-400" /> Reported Symptoms</h4>
                                                        ...
                                                        <div className="mt-3 text-xs text-white/30"><span className="font-bold">Duration:</span> {triageData.duration || 'Not specified'} · <span className="font-bold">Severity:</span> {triageData.severity || 'N/A'}</div>
                                                    </div>

                                                    {/* Possible Conditions */}
                                                    <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
                                                        <h4 className="font-black text-white/60 text-xs uppercase tracking-wider mb-3 flex items-center gap-2"><Zap className="h-4 w-4 text-violet-400" /> Possible Conditions</h4>
                                                        <div className="space-y-2">
                                                            {triageData.possibleConditions?.map((c: string, i: number) => (
                                                                <div key={i} className="flex items-center gap-2 text-sm text-white/60"><div className="h-1.5 w-1.5 rounded-full bg-violet-400 shrink-0" />{c}</div>
                                                            ))}
                                                        </div>
                                                    </div>
                                                    ...
                                                    {/* Recommended Tests */}
                                                    <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
                                                        <h4 className="font-black text-white/60 text-xs uppercase tracking-wider mb-3 flex items-center gap-2"><FlaskConical className="h-4 w-4 text-amber-400" /> Recommended Tests</h4>
                                                        <div className="flex flex-wrap gap-2">
                                                    ...
                                                    {/* Medical History */}
                                                    <div className="bg-white/[0.03] border border-white/5 rounded-xl p-5">
                                                        <h4 className="font-black text-white/60 text-xs uppercase tracking-wider mb-3 flex items-center gap-2"><FileText className="h-4 w-4 text-cyan-400" /> Medical History</h4>
                                                        <div className="space-y-2 text-sm text-white/50">
                                                            <div><span className="font-bold text-white/30 text-xs">PMH: </span>{triageData.pastMedicalHistory || 'None reported'}</div>
                                                            <div><span className="font-bold text-white/30 text-xs">Medications: </span>{triageData.currentMedications || 'None'}</div>
                                                            <div><span className="font-bold text-white/30 text-xs">Allergies: </span>{triageData.allergies || 'NKDA'}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-4">
                                                    <h4 className="font-black text-white/70 text-xs uppercase tracking-[0.15em] flex items-center gap-2"><Brain className="h-4 w-4 text-teal-400" /> AI Clinical Assessment — Detailed Report</h4>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        if (!hasParsed) {
                                                            ...
                                                            return (
                                                                <div className="bg-gradient-to-br from-white/[0.04] to-white/[0.02] border border-white/10 rounded-2xl p-6">
                                                                    <pre className="text-sm text-white/60 whitespace-pre-wrap leading-relaxed font-sans">{summary}</pre>
                                                                </div>
                                                            );
                                                        }
                                                        ...
                                                        return sections.filter(s => parsed[s.key]).map(section => (
                                                            <div key={section.key} className={`border rounded-xl p-5 ${colorMap[section.color]} transition-all hover:border-opacity-40`}>
                                                                ...
                                                                <div className="text-sm text-white/60 leading-relaxed whitespace-pre-wrap">{parsed[section.key]?.trim()}</div>
                                                            </div>
                                                        ));
                                                    })()}
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between bg-white/[0.02] border border-white/5 rounded-xl p-4">
                                                    <p className="text-[11px] text-white/25 font-medium max-w-lg">⚕️ This is an AI-assisted clinical assessment. All findings should be verified through physical examination and diagnostic confirmation. Final clinical decision rests with the attending physician.</p>
                                ...
                                {activeTab === 'notes' && (
                                    <div className="max-w-3xl space-y-6">
                                        {activePatient.status === 'Admitted' ? (<>
                                            ...
                                            <div><label className={labelCls}>Note Type</label><select value={medicalNoteType} onChange={e => setMedicalNoteType(e.target.value)} className={inputCls}><option className="bg-[#131A2E]">Routine Check</option><option className="bg-[#131A2E]">Admission Note</option><option className="bg-[#131A2E]">Nursing</option><option className="bg-[#131A2E]">Discharge Advice</option></select></div>
                                ...
                                {activeTab === 'history' && (
                                    <div className="max-w-4xl space-y-6">
                                        <h3 className="font-black text-white/80 mb-4 flex items-center gap-2 text-lg"><History className="h-5 w-5 text-violet-400" /> Patient History</h3>
                                        {loadingHistory ? <div className="text-center py-12 text-white/30 font-bold">Loading history...</div> : history.length === 0 ? (
                                            <div className="bg-white/5 border border-dashed border-white/10 rounded-xl p-8 text-center text-white/25 text-sm font-bold">No previous records found.</div>
                                        ) : (<div className="space-y-4">{history.map((record, i) => (
                                            <div key={i} className="bg-white/[0.03] border border-white/5 p-5 rounded-xl hover:border-teal-500/20 transition-all">
                                                <div className="flex justify-between items-start mb-3">
                                                    <div><p className="font-bold text-teal-400 text-base">{record.diagnosis || 'No Diagnosis'}</p><p className="text-xs text-white/30 mt-1">{new Date(record.created_at).toLocaleDateString()} • {record.doctor_name || 'Dr. Unknown'}</p></div>
                                                    <div className="bg-white/5 text-white/25 text-[10px] uppercase font-black px-2 py-1 rounded-lg border border-white/5">#{record.appointment_id}</div>
                                                </div>
                                                <p className="text-sm text-white/50 whitespace-pre-wrap leading-relaxed">{record.doctor_notes}</p>
                                            </div>
                                        ))}</div>)}
                                    </div>
                                )}
                                ...
                                {activeTab === 'lab' && (
                                    <div className="max-w-3xl space-y-8">
                                        <div className="bg-violet-500/5 p-6 rounded-2xl border border-violet-500/10">
                                            ...
                                            <div className="flex gap-4">
                                                <select value={selectedTest} onChange={e => setSelectedTest(e.target.value)} className={`flex-1 ${inputCls}`}>
                                                    <option value="" className="bg-[#131A2E]">Select Test Type...</option><option value="Complete Blood Count (CBC)" className="bg-[#131A2E]">Complete Blood Count (CBC)</option><option value="Lipid Profile" className="bg-[#131A2E]">Lipid Profile</option><option value="Dengue NS1 Antigen" className="bg-[#131A2E]">Dengue NS1 Antigen</option><option value="Liver Function Test" className="bg-[#131A2E]">Liver Function Test</option>
                                                </select>
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-4">
                                                <h3 className="font-black text-white/80 text-lg">Lab History</h3>
                                            ...
                                            <div className="space-y-3">
                                                {labOrders.length === 0 ? <div className="bg-white/5 border border-dashed border-white/10 rounded-xl p-8 text-center text-white/25 text-sm font-bold">No lab orders found.</div> : labOrders.map(order => (
                                                    <div key={order.id} className="flex items-center justify-between bg-white/[0.03] border border-white/5 p-4 rounded-xl hover:border-teal-500/20 transition-all">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div><p className="font-bold text-white/80">{order.test_type}</p><p className="text-xs text-white/30 flex items-center gap-1 mt-0.5">{order.barcode && <span className="font-mono bg-white/5 px-1 rounded border border-white/5">#{order.barcode}</span>}<span className="text-white/15">•</span>{new Date(order.created_at).toLocaleDateString()}</p></div>
                                                        </div>
                                                        <div className="text-right">
                                                            ...
                                                            {order.result_value && <p className="text-xs text-white/50 mt-1 font-mono bg-white/5 px-2 py-0.5 rounded-lg border border-white/5 inline-block">{order.result_value}</p>}
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                {activeTab === 'pharmacy' && (
                                    <div className="max-w-4xl space-y-6">
                                        <div className="flex gap-8">
                                            <div className="flex-1 space-y-6">
                                                <div className="bg-white/[0.03] p-6 rounded-2xl border border-white/5">
                                                    <div className="flex justify-between items-center mb-6">
                                                        <h3 className="font-black text-white/80 flex items-center gap-2"><Pill className="h-5 w-5 text-teal-400" /> Prescribe Medicine</h3>
                                                        {pharmacyCart.length > 0 && <button onClick={handlePrintPrescription} className="text-xs font-bold text-white/30 flex items-center gap-1 hover:text-white/60"><Printer className="h-3 w-3" /> Preview Rx</button>}
                                                    </div>
                                                    <div className="flex gap-3 mb-4">
                                                        <select value={selectedMedicine} onChange={e => setSelectedMedicine(e.target.value)} className={`flex-[2] ${inputCls}`}>
                                                            <option value="" className="bg-[#131A2E]">Select Medicine...</option>{medicines.map((m: any) => <option key={m.id} value={m.brand_name} className="bg-[#131A2E]">{m.brand_name} (₹{m.price_per_unit})</option>)}
                                                        </select>
                                                    ...
                                                    <div className="bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden">
                                                        <div className="bg-white/5 px-4 py-3 text-[10px] font-black text-white/25 uppercase tracking-[0.15em] flex justify-between border-b border-white/5"><span>Current Rx Cart</span><span>{pharmacyCart.length} Items</span></div>
                                                        {pharmacyCart.length === 0 ? <div className="p-8 text-center text-white/20 text-sm font-bold">Add medicines to create prescription</div> : (
                                                            <div className="divide-y divide-white/5">{pharmacyCart.map((item, idx) => (
                                                                <div key={idx} className="p-4 flex justify-between items-center hover:bg-white/[0.02] transition-colors">
                                                                    <div><span className="font-bold text-white/80 text-sm block">{item.name}</span><span className="text-xs text-white/30">Qty: {item.qty}</span></div>
                                            ...
                                            {pharmacyOrderResult && (
                                                <div className="flex-1 bg-gradient-to-br from-[#131A2E] to-[#0F1425] border border-white/5 rounded-2xl p-6 h-fit">
                                                    <div className="flex items-center gap-3 mb-6 text-emerald-400 font-bold border-b border-white/5 pb-4">
                                                    ...
                                                    <div className="space-y-4 text-sm">
                                                        <div className="flex justify-between py-2 border-b border-white/5"><span className="text-white/40 font-medium">Total Requested</span><span className="font-bold bg-white/5 px-2 py-0.5 rounded-lg text-white/60 border border-white/5">{pharmacyOrderResult.bill_summary?.total_items_requested}</span></div>
                                                        <div className="flex justify-between py-2 border-b border-white/5"><span className="text-white/40 font-medium">Items Dispensed</span><span className="font-bold text-teal-400 bg-teal-500/10 px-2 py-0.5 rounded-lg">{pharmacyOrderResult.bill_summary?.items_dispensed}</span></div>
                                                        <div className="flex justify-between py-2 border-b border-white/5"><span className="text-white/40 font-medium">Unavailable</span><span className="font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-lg">{pharmacyOrderResult.bill_summary?.items_missing}</span></div>
                                                        <div className="pt-6 flex justify-between items-end"><span className="font-bold text-white/20 uppercase text-[10px] tracking-[0.15em] block">Total Bill</span><span className="text-3xl font-black text-white tracking-tight">₹{pharmacyOrderResult.bill_summary?.total_amount_to_collect}</span></div>
                                                        <button onClick={handlePrintPrescription} className="w-full mt-4 py-3 bg-white/5 text-white/80 rounded-xl font-bold hover:bg-white/10 border border-white/10 flex items-center justify-center gap-2"><Printer className="h-4 w-4" /> Print Receipt</button>
                                                        <button onClick={() => setPharmacyOrderResult(null)} className="w-full py-3 text-white/25 hover:text-white/50 text-xs font-bold">New Order</button>
                                                    </div>
                                                </div>
                                            )}
                                        </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-white/30 relative">
                        <div className="z-10 flex flex-col items-center">
                            <div className="h-24 w-24 bg-white/5 rounded-full mb-6 border border-white/5 flex items-center justify-center"><Users className="h-10 w-10 text-white/15" /></div>
                            <h2 className="text-xl font-black text-white/50 mb-2">Ready for Consultation</h2>
                            <p className="text-white/25 max-w-xs text-center font-medium">Select a patient from the queue to start consultation.</p>
                        </div>
                    </div>
                )}
            </main>

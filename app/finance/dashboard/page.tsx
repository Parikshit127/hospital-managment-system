...
export default function FinanceDashboard() {
    ...
    const handleRazorpayPayment = async () => {
        ...
        try {
            ...
            const rzp = new window.Razorpay({
                ...
                name: orderData.data.hospital_name || 'Hospital', description: `Payment for ${paymentModal.invoice_number}`,
    ...
    const kpiCards = [
        ...
        { label: 'Advances (Unbilled)', value: `${INR}${(stats?.unbilledAdvances || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`, sub: 'Cash on draft bills', icon: <Wallet className="h-3.5 w-3.5 text-violet-400" />, color: 'violet', subIcon: <Receipt className="h-3 w-3" />, drillType: 'drafts' as DrillDownType },

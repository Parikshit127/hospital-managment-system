...
import {
    Stethoscope, Calendar, Clock, Users, Activity, UserPlus, Filter,
    Loader2, Phone, ChevronRight, LayoutDashboard, Settings2,
} from 'lucide-react';
import Link from 'next/link';
import { ModuleHubLayout } from '../components/ModuleHubLayout';
import { OPDSettingsContent } from '../opd-settings/OPDSettingsContent';
...
export default function AdminOPDHub() {
    ...
    return (
        <ModuleHubLayout
            moduleKey="opd"
            moduleTitle="OPD Module"
            moduleDescription="Outpatient department operations & configuration"
            moduleIcon={<Stethoscope className="h-5 w-5" />}
            tabs={TABS}
            activeTab={activeTab}
        ...
        >
            {activeTab === 'dashboard' && (
                <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                                {stats.byDepartment.map((dept: any) => (
                                    <button key={dept.department}
                                        ...
                                        className={`p-3 rounded-xl border text-center transition-all ${deptFilter === dept.department ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'
                                            }`}>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {(deptFilter || statusFilter) && (
                            <button onClick={() => { setDeptFilter(''); setStatusFilter(''); }}
                                className="text-xs font-medium text-blue-600 hover:underline">Clear filters</button>
                        )}
                    </div>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {loading ? (
                                        ...
                                        <tr key={appt.appointment_id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-gray-500">{appt.age || '-'} / {appt.gender || '-'}</td>
            ...
            {activeTab === 'settings' && <OPDSettingsContent />}
        </ModuleHubLayout>
    );
}

...
import { getAvailableDoctors, getAvailableSlots, bookAppointment } from '../actions';
...
export default function BookAppointmentPage() {
    ...
    async function handleConfirmBooking() {
        ...
        setBookingError('');
        try {
            const res = await bookAppointment(selectedSlot.id, selectedDoctor.id, selectedDate, reason);
            if (res.success) {
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            {step === 1 && (
                <div className="space-y-4">
                    {/* Specialty Filter */}
                    <div className="flex items-center gap-3">
                        <label className="text-sm font-bold text-gray-700">Filter by specialty:</label>
                        <select
                    ...
                    {loadingDoctors ? (
                        ...
                        <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center">
                            ...
                            <p className="text-lg font-bold text-gray-700">No doctors available</p>
                    ...
                    {selectedDoctor && (
                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-bold text-emerald-900">Dr. {selectedDoctor.name}</p>
                                    <p className="text-sm text-emerald-700">{selectedDoctor.specialty}</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setStep(1)}

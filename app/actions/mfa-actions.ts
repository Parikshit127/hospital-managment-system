...
export async function setupMFA(userId: string) {
    try {
        ...
        // Store secret (not yet enabled until verified) — user_mfa has no organizationId
        await db.user_mfa.upsert({
...
export async function enableMFA(userId: string, token: string) {
    try {
        ...
        const mfaRecord = await db.user_mfa.findUnique({ where: { user_id: userId } });
...
export async function disableMFA(userId: string) {
    try {
        ...
        await db.user_mfa.deleteMany({ where: { user_id: userId } });

...
import {
    getWardsWithBeds, getAllBeds, getIPDAdmissions, getIPDStats,
...
export default function IPDDashboard() {
    ...
    const [admitForm, setAdmitForm] = useState({ bed_id: '', ward_id: '', diagnosis: '', doctor_name: '' });
    ...
    const [noteForm, setNoteForm] = useState({ type: 'Routine Check', details: '' });
    ...
    const loadData = async () => {
        ...
        } catch (err) { console.error('IPD load error:', err); }
        setLoading(false);
    };

    useEffect(() => { loadData(); }, [admissionFilter]);
    ...
    const handleAdmit = async () => {
        setAdmitError('');
        if (!selectedPatient || !admitForm.bed_id || !admitForm.ward_id || !admitForm.diagnosis) {
            setAdmitError('Please fill in all required fields: patient, ward, bed, and diagnosis.');
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                ...
                setAdmitForm({ bed_id: '', ward_id: '', diagnosis: '', doctor_name: '' });
                setAdmitError('');
                loadData();
            } else {
                setAdmitError(res.error || 'Failed to admit patient. Please try again.');
            }
        } catch (err: any) {
            console.error(err);
    ...
    const handleDischarge = async () => {
        ...
        try {
            const res = await dischargePatientIPD(dischargeModal.admission_id, dischargeNotes);
            if (res.success) {
    ...
    const handleAddNote = async () => {
        ...
        await addMedicalNote(noteModal.admission_id, noteForm.type, noteForm.details);
        setNoteModal(null);
                                        ...
                                        {wards.length === 0 ? (
                                            ...
                                            <div key={ward.ward_id} className="p-4 bg-gray-50 border border-gray-200 rounded-xl">
                                                ...
                                                <div className="flex gap-3 mt-2 text-[10px] font-bold">
                                                    ...
                                                    <span className="text-amber-400">{ward.maintenance} maint.</span>
                                                    {ward.cleaning > 0 && <span className="text-cyan-400">{ward.cleaning} cleaning</span>}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div key={ward.ward_id} className="bg-white border border-gray-200 shadow-sm rounded-2xl overflow-hidden">
                                        ...
                                        <div className="p-5 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
                                            {ward.beds.map((bed: any) => (
                                                <div key={bed.bed_id} className={`relative group border rounded-xl p-3 text-center cursor-pointer transition-all ${getBedStatusColor(bed.status)}`}>
                            ...
                            <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select value={admitForm.bed_id} onChange={e => {
                                            ...
                                            setAdmitForm({
                                                ...admitForm,
                                                bed_id: bedId,
                                                ward_id: selectedBed ? String(selectedBed.ward_id) : admitForm.ward_id,
                                            });
                                            setAdmitError('');
                                        }}
                                            className="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl text-sm text-gray-900 focus:outline-none focus:border-violet-500/50">
                                ...
                                {admitError && (
                                    <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-3 flex items-start gap-2">
                                        ...
                                        <p className="text-xs text-rose-400 font-medium">{admitError}</p>
                                    </div>
                                )}

                                <button onClick={handleAdmit} disabled={admitLoading || !admitForm.bed_id || !admitForm.ward_id || !admitForm.diagnosis}
                                    ...
                                    {admitLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus className="h-4 w-4" />}
                                    Admit Patient
                                </button>
                            </div>
                        )}
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                    <div className="bg-white border border-gray-200 shadow-sm rounded-2xl w-full max-w-md p-6 space-y-5">
                        ...
                        <button onClick={handleDischarge} disabled={dischargeLoading}
                            ...
                            {dischargeLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle className="h-4 w-4" />}
                            Confirm Discharge
                        </button>
                    </div>
                </div>

...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    if (!reportDef) {
        return { title: 'Report Not Found — IPD | HospitalOS' };
    }
    return {
        title: `${reportDef.name} — IPD MIS | HospitalOS`,
        description: reportDef.description,
    };
}

export default async function IPDMISViewerPage({ params, searchParams }: PageProps) {
    const { reportId } = await params;
